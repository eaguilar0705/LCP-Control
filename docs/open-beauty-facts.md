...
```sh
node scripts/audit_openbeautyfacts.mjs --catalog private-data/database-import/products.json
node scripts/audit_openbeautyfacts.mjs --offline
node --test scripts/openbeautyfacts_matching.test.mjs
```
...
```sh
node scripts/fetch_openbeautyfacts_perfumes.mjs
node scripts/audit_openbeautyfacts.mjs --offline --supplement private-data/openbeautyfacts/category-search.json
```
